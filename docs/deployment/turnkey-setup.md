...
| `read_assets` `write_assets` | Jira → **Granular scopes** (CMDB) **+** Jira Service Management (classic) | `read:cmdb-object:jira` `write:cmdb-object:jira` `read:cmdb-schema:jira` `write:cmdb-schema:jira` `read:cmdb-type:jira` `write:cmdb-type:jira` `read:cmdb-attribute:jira` `write:cmdb-attribute:jira` `delete:cmdb-object:jira` `delete:cmdb-schema:jira` `delete:cmdb-type:jira` `delete:cmdb-attribute:jira` `import:import-configuration:cmdb` **plus** `read:servicedesk-request` — Assets calls are OAuth-authed, and before any of them can run gojira resolves the workspaceId with an OAuth-authed `GET /rest/servicedeskapi/assets/workspace` (`src/atlassian/assetsWorkspace.ts`). Without the JSM read scope that discovery call fails and **every** `assets.*` tool errors, CMDB scopes or not. |

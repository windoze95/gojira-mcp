...
| `/metrics/usage` | GET | Aggregated per-tool/per-user usage counters (`?days=N`, default 30, max 400) plus an `allTime` cumulative section not limited by retention. Only registered when `GOJIRA_METRICS_TOKEN` is set, and requires it as a bearer token (constant-time compare) — 401 otherwise. 404 when unconfigured. |

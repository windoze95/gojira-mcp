...
Total tools registered: **65** (**162** operations).
...
### `read_jsm_admin` (4 tools, 18 operations)
...
- **Requires cloudId:** yes
- **Input:**
  - `spaceIdOrKey`: string — Jira service space id or key (the REST adapter maps this to projectIdOrKey)
  - `serviceDeskId`: string — JSM service desk id
  - `requestTypeId`: string — JSM request type id
  - `automationRuleIds`: array<string>
  - `trackingWorkItemIdOrKey`: string
  - `expectForm`: boolean (default: true)
  - `expectAutomation`: boolean (default: true)
  - `expectTrackingWorkItem`: boolean (default: false)
  - `maxAutomationRules`: number (default: 200)
...
### `read_workitems` (2 tools, 3 operations)

### `workitems.read`

Read a Jira work item (getWorkItem) or its comments (listComments).
...
Get one work item by id or key.

- **Destructive:** no
- **Input:**
  - `workItemIdOrKey`: string — Work item id or key (the REST adapter maps this to issueIdOrKey)
  - `fields`: array<string>
  - `expand`: array<string>

#### op: `listComments`

List comments on one work item, including entity properties when permitted.
...
- **Input:**
  ...
  - `orderBy`: enum("created" | "-created" | "+created") (default: "created")
...
- **Input:**
  ...
  - `fields`: array<string> (default: ["summary","status","assignee","issuetype","project"])
  - `expand`: array<string>
...
### `write_workitems` (1 tools, 3 operations)

### `workitems.manageComment`
...
Create a comment. JSM visibility defaults to internal. Revert deletes the created comment.

- **Destructive:** yes
- **Input:**
  - `workItemIdOrKey`: string — Work item id or key (the REST adapter maps this to issueIdOrKey)
  - `bodyText`: string — Plain text comment body; converted to Atlassian Document Format
  - `bodyDocument`: record<string, unknown> — Atlassian Document Format comment body; use exactly one of bodyText or bodyDocument
  - `visibility`: enum("internal" | "public") — JSM customer visibility. Internal is the safe default for newly created comments. (default: "internal")

#### op: `updateComment`

Update a comment body and optionally its JSM visibility. Revert restores the captured comment.
...
- **Input:**
  ...
  - `bodyText`: string — Plain text comment body; converted to Atlassian Document Format
  - `bodyDocument`: record<string, unknown> — Atlassian Document Format comment body; use exactly one of bodyText or bodyDocument
  - `visibility`: enum("internal" | "public") — JSM customer visibility. Internal is the safe default for newly created comments.
...
- **Input:**
  - `workItemIdOrKey`: string — Work item id or key (the REST adapter maps this to issueIdOrKey)
  - `marker`: string — Stable idempotency marker stored as a Jira comment entity property
  - `bodyText`: string — Plain text comment body; converted to Atlassian Document Format
  - `bodyDocument`: record<string, unknown> — Atlassian Document Format comment body; use exactly one of bodyText or bodyDocument
  - `visibility`: enum("internal" | "public") — JSM customer visibility. Internal is the safe default for newly created comments.

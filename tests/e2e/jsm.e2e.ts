...
describe.skipIf(noCreds)("e2e: JSM admin", () => {
  ...
  it("lists service desks and reads one", async () => {
    ...
    const one = await h.call<{ id: string }>("jsm.readServiceDesk", { op: "getServiceDesk", serviceDeskId });
    expect(one.id).toBe(serviceDeskId);
  });

  it("reads request types, queues, organizations, KB", async () => {
    const rts = await h.call<{ values: unknown[] }>("jsm.readServiceDesk", { op: "listRequestTypes", serviceDeskId });
    expect(rts.values.length).toBeGreaterThan(0);
    const queues = await h.call<{ values: Array<{ id: string }> }>("jsm.readSupport", { op: "listQueues", serviceDeskId });
    expect(Array.isArray(queues.values)).toBe(true);
    await h.call("jsm.readSupport", { op: "listJsmOrganizations" });
    await h.call("jsm.readSupport", { op: "searchKnowledgeBaseArticles", serviceDeskId, query: "vpn" });
  });

  it("request type create → dry-run gate → delete round-trip", async () => {
    // find an issue type to hang the request type on
    const rts = await h.call<{ values: Array<{ issueTypeId: string }> }>("jsm.readServiceDesk", { op: "listRequestTypes", serviceDeskId });
    const issueTypeId = rts.values[0].issueTypeId;

    // dry-run first: no commit → no mutation
    const dry = await h.call<{ dry_run?: boolean }>("jsm.manage", {
    ...
    const after = await h.call<{ values: Array<{ id: string; name: string }> }>("jsm.readServiceDesk", {
      op: "listRequestTypes",
      serviceDeskId,
    });
    const probe = after.values.find((v) => v.name === "gojira-e2e-rt");
    expect(probe).toBeDefined();
    const del = await h.call<{ ok: boolean }>("jsm.delete", {

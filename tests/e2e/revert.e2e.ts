...
describe.skipIf(noCreds)("e2e: revertOperation", () => {
  ...
  it("reverts a form-template create (delete via the API-token client)", async () => {
    const created = await h.call<{ journal_id: string; form: { id: string } }>("forms.manageTemplate", {
    ...
    // Journal entries carry the collapsed tool name; the op rides in request.op.
    expect(dry.original.tool).toBe("forms.manageTemplate");
  ...
  it("refuses to revert an irreversible op", async () => {
    ...
    const rts = await h.call<{ values: Array<{ issueTypeId: string }> }>("jsm.readServiceDesk", {
      op: "listRequestTypes",
      serviceDeskId: (await h.call<{ values: Array<{ id: string }> }>("jsm.readServiceDesk", { op: "listServiceDesks" })).values[0].id,
    });
    const serviceDeskId = (await h.call<{ values: Array<{ id: string }> }>("jsm.readServiceDesk", { op: "listServiceDesks" })).values[0].id;
    await h.call("jsm.manage", {
    ...
    const list = await h.call<{ values: Array<{ id: string; name: string }> }>("jsm.readServiceDesk", {
      op: "listRequestTypes",
      serviceDeskId,
    });
    const probe = list.values.find((v) => v.name === "gojira-e2e-revert-rt")!;
    const del = await h.call<{ ok: boolean; journal_id: string }>("jsm.delete", {
